...
import {sendEventOnNetwork} from "../../request_service.js";
...
class RemoteManager {
    ...
    listenLocalEvents() {
        bus.observe("event.network",data => {
    ...
    readEvent(event) {
        ...
        switch (event.kind) {
            ...
            case "disconnected":
                this.debug("DISCONNECTED", event);
                this.disconnect(event);
                break; //
            case "connected":
                this.debug("CONNECTED", event);
                this.connect(event, joueur);
                return; //
            case "move":
                this.debug("MOVES", event);
                joueur.moveTo(event.nb);
                break;   //
            case "moveTo":
                this.debug("MOVES", event);
                joueur.joueSurCase(GestionFiche.getById(event.to));
                break;   //
            case "tax":
                this.debug("TAX", event);
                joueur.setArgent(joueur.montant - event.montant);
                break;    //
            case "earn":
                this.debug("EARN", event);
                joueur.setArgent(joueur.montant + event.montant);
                break;   //
            case "hypotheque":
                this.debug("HYPOTHEQUE", event);
                GestionFiche.getById(event.terrain).doHypotheque();
                break; //
            case "leverHypotheque":
                this.debug("LEVER HYPOTHEQUE", event);
                GestionFiche.getById(event.terrain).doLeveHypotheque();
                break;  //
            case "change":
                this.debug("CHANGE");
                GestionJoueur._select(joueur);
                break;   //
            case "jail":
                this.debug("JAIL");
                joueur.goPrison();
                break;
            case "buyHouse":
                this.debug("BUY HOUSE");
                GestionFiche.getById(event.terrain).setNbMaison(event.nb);
                break;
            case "loyer":
                this.debug("LOYER");
                bus.send('monopoly.payerLoyer', {joueur: joueur, maison: GestionFiche.getById(event.maison)});
                break;
            case "exit":
                this.debug("EXIT");
                bus.send('monopoly.exit', {joueur: joueur});
                break;
            case "message":
                this.debug("GOT MESSAGE", event);
                bus.send(event.name, {
                    joueur: joueur,
    ...
    queueEvents = {
        ...
        detectEndMove() {
            bus.observe('move.end', () => this.end({kind: 'moveTo'}, true));
        }
    }
}
...
export class MasterRemoteManager extends RemoteManager {
    ...
    create(nbJoueurs, nbRobots, playerName, players, argentDepart, montantDepart) {
        ...
        if (nbRemote > 0) {
            bus.send("monopoly.waitingPlayers", {nb: nbRemote, idGame: this.game});
        }
        return players;
    }
    ...
    readMoreEvent(event, joueur) {
        ...
        switch (event.kind) {
            ...
            case "end":
                ...
                break;
            default:
                super.readMoreEvent(event, joueur);
                break;
        }
    }
    ...
    getPlayersInformations() {
        return GestionJoueur.joueurs.map(j => {
            return {
                ...
                maisons: j.maisons.maisons.map(m => {
                    return {
                        hotel: m.hotel,
                        axe: m.axe,
                        pos: m.pos,
...
class EventSender {
    ...
    sendEvent(event) {
        sendEventOnNetwork(this.url, event);
        if (this.isMaster) {
