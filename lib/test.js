	 ...
	 function buildMaison(){
	   ...
	   var l = lignes[joueurCourant.pion.axe];
	   var p = joueurCourant.pion.pion.position();
	   var id = 'maison_' + fiche.axe + "-" + fiche.pos + "-" + fiche.nbMaison;
	   if(fiche.nbMaison == 4){
		// hotel
	 ...
	 function getNextPos(axe,position){
		  position++;
		  if(position == 10){
			  axe=(axe+1)%4;
			  position = 0;
		  }
		  return {"position":position,"axe":axe};
	 }
	 ...
	 function Comportement(risque){
		  ...
		  this.calculRisque = function(joueur){
			  ...
			  var stats = 0;
			  for(var i = 1 ; i <= 12 ; i++){ 
				  var pos = getNextPos(axe,position);
				  axe = pos.axe;
				  position = pos.position;
				  var fiche = fiches[axe + "-" + position];
	 ...
	 function Joueur(numero,nom){
	   ...
	   this.getFichePosition = function(){
		return fiches[this.pion.axe + "-" + this.pion.position];
	   }
	 ...
	 function Pion(color,joueur){
	   ...
	   this.goto = function(axe,pos,call){
		  // decalage
		  var center = fiches[this.axe + "-" + this.position].case.getCenter();
		  this.pion.x = center.x;
		  this.pion.y = center.y;
		  console.log(joueurCourant.numero + " va a " + axe + "-" + pos);
		this.gotoCell(axe,pos,call);
	   }
	   
	   // Si on passe par la case depart, on prend 20000 Francs
	   this.treatCaseDepart = function(axeCible,posCible){
		  if(!this.joueur.isEnPrison() && this.position == 0 && this.axe == 2 && this.position!=posCible && this.axeCible != this.axe){
			  this.joueur.gagner(20000);
		  }
	   }
	   
	   this.goDirectToCell = function(axe,pos){
		  // On calcule la fonction affine
		  var p1 = fiches[this.axe + "-" + this.position].case.getCenter()
		  var p2 = fiches[axe + "-" + pos].case.getCenter()
	   ...
	   this.gotoCell = function(axeCible,posCible,callback){
		   // Cas de la fin
		 if(this.axe == axeCible && this.position == posCible){
			  // On decale le pion
			  var decalage = fiches[this.axe + "-" + this.position].case.decalagePion();
		 ...
		 this.treatCaseDepart(axeCible,posCible);
		 var pas = 5;
		...
		var interval = setInterval(function(){
		   if(distance > 0){
			 ...
			 clearInterval(interval);
			 _self.gotoCell(axeCible,posCible,callback);
		   }
		},30);
	   }
	   ...
	   this.toNextCase = function(){
		 ...
		 return fiches[this.axe + "-" + this.position].case.getCenter();
	   }
	 }
	 
	 function CarteActionSpeciale(titre,actionSpeciale,axe,pos){
	   this.titre = titre;
	   this.actionSpeciale = actionSpeciale;
	   
	   this.case = new CaseSpeciale(axe,titre);
		Drawer.add(this.case);
	 ...
	 function CarteSpeciale(titre,montant,axe,pos,img){
	   this.case = new Case(pos,axe,null,titre,"F. " + montant,img);
		Drawer.add(this.case);
	   this.action = function(){
	 ...
	 function Chance(axe,pos){
	    this.case = new Case(pos,axe,null,"Chance",null,{src:"interrogation.png",width:50,height:60});
	    Drawer.add(this.case);
	   this.action = function(){
	 ...
	 function CaisseDeCommunaute(axe,pos){
	  this.case = new Case(pos,axe,null,"Caisse de communaut�",null,{src:"banque.png",width:50,height:50});
	  Drawer.add(this.case);
	    this.action = function(){
  ...
  function Case(pos,axe,color,title,prix,img){
	...
	this.getNbJoueurs = function(){
	  ...
	  for(var i  = 0 ; i < joueurs.length ; i++){
		  if(joueurs[i].pion.axe == this.axe && joueurs[i].pion.position == this.pos){
			  count++;
		  }
	  }
	  return count;	
	...
	this.init();
	 ...
	 function Fiche(axe,pos,colors,nom,achat,loyer,loyer1,loyer2,loyer3,loyer4,loyerHotel,prixMaison,img){
	   ...
	   this.case = new Case(pos,axe,this.color,this.nom,"F. " + achat,img);
	   Drawer.add(this.case);
		  ...
		  this.getButtons = function(){
			  if(this.statut == ETAT_LIBRE){
				  if(joueurCourant.montant<this.achat){
					  ...
					  return {
					  "Acheter":function(){
						  var id = joueurCourant.pion.axe + "-" + joueurCourant.pion.position;
	 ...
	 function FicheGare(axe,pos,color,nom,achat,loyer1,loyer2,loyer3,loyer4){
	   Fiche.call(this,axe,pos,color,nom,achat,loyer1,loyer2,loyer3,loyer4,null,null,null,{src:"train.png",width:40,height:50});
	 ...
	 function FicheCompagnie(axe,pos,color,nom,achat,loyer1,loyer2){
	   Fiche.call(this,axe,pos,color,nom,achat,loyer1,loyer2);
	   this.fiche = $('#ficheCompagnie');
	   this.type = "compagnie";
	   this.constructible = false;
	 ...
	 function construireMaisons(modeBanqueroute){
	   ...
	   for(var i = 0 ; i < maisons.length ; i++){
		...
		if(maisons[i].hotel == true){
		  ...
		  var id = maisons[i].axe + "-" + maisons[i].pos;
	   ...
	   $('#achatMaisons').dialog('option','buttons',{
		...
		var equilibre = true;
		  ...
		  var currentAxe = joueurCourant.pion.axe;
	 ...
	 function doActions(){
		  var fiche = fiches[joueurCourant.pion.axe + "-" + joueurCourant.pion.position];
	...
	function closeFiche(){
	  ...
	  function rand(){
	   ...
	   function openDetailFiche(fiche,input){
		 if(currentFiche!=null && currentFiche.axe == fiche.axe && currentFiche.pos == fiche.pos){
